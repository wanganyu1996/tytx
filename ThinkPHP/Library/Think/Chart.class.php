...
class Chart {
  ...
  */
  static function create3dpie($title,$data=array(),$size=40,$height=100,$width=80,$legend=array(),$slice=0){
	  ...
	  $graph->legend->SetFont(FF_SIMSUN,FS_BOLD,20);
      //编码转化
   ...
   * 设置柱状图
   */
  static function createcolumnar($title,$data=array(),$size=80,$height=100,$width=80,$legend=array()){
  	    //载入文件
		vendor("Jpgraph.jpgraph");
		...
		$graph->yaxis->SetFont(FF_SIMSUN,FS_BOLD,20);
		...
		$graph->xaxis->SetFont(FF_SIMSUN,FS_BOLD,20);
  ...
  */
  ...
  function createmonthline($title,$data=array(),$size=40,$height=100,$width=80,$legend=array()){
	...
	$graph->SetShadow();
	$graph->title->Set(iconv('utf-8', 'gb2312',"{$title}"));
	$graph->title->SetFont(FF_SIMSUN,FS_NORMAL,20);
	$graph->SetFrame(false,'#ffffff',0);//去掉周围的边框
	$graph->xaxis->SetFont(FF_SIMSUN,FS_NORMAL,20);
	$graph->yaxis->SetFont(FF_SIMSUN,FS_NORMAL,20);
